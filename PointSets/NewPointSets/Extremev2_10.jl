using JuMP
using Gurobi
log_file_path = "/scratchbeta/clementf/Ext10.txt"
...
# constraints (5b)
@constraint(Sparse, [i in 1:m+1, j in 1:m+1, k in 1:i, l in 1:j], -1/m* sum(a[u,v] for u in k+1:i-1, v in l+1:j-1) + (x[i]-x[k])*(y[j]-y[l]) <= z )
...
output_file= open("/scratchbeta/clementf/OCE_10.jl", "w")
write(output_file, "x = ")
